node ('swarm'){
  ...
  withEnv(["PATH+MAVEN=${tool 'maven-3.3.1'}/bin"]) {
    ...
    def fabricMavenPluginVersion = '2.2.61'
    ...
    sh "mvn clean install org.apache.maven.plugins:maven-deploy-plugin:2.8.2:deploy org.jolokia:docker-maven-plugin:${dockerMavenPluginVersion}:build -Dfabric8.dockerUser=fabric8/ -Ddocker.registryPrefix=${registry}"
